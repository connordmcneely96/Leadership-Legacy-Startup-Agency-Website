...
import { rectSortingStrategy, SortableContext, useSortable, arrayMove } from '@dnd-kit/sortable'
      ...
      {view === 'board' ? (
        <DndContext onDragEnd={handleDragEnd}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {(loading ? (['todo', 'in_progress', 'review', 'done'] as ColumnKey[]) : (Object.keys(columns) as ColumnKey[])).map(
              (column) => (
                <TaskColumn
                  key={column}
                  ...
                  tasks={loading ? mockTasks[column] : columns[column]}
                />
              )
            )}
          </div>
